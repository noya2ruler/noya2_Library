...
data:
  ...
  code: "#pragma once\n\n#include\"simple_tree.hpp\"\n\nnamespace noya2 {\n\nstruct\
    \ hld_tree {\n    internal::csr<int> g;\n    hld_tree () {}\n    hld_tree (int\
    \ _n, int _root = 0) : g(_n,(_n - 1)*2), n(_n), root(_root) {}\n    hld_tree (simple_tree\
    \ _g, int _root = 0) : g(_g.g), n(_g.g.n), root(_root){\n        build();\n  \
    \  }\n\n    void add_edge(int u, int v){\n        g.add(u, v);\n        int id\
    \ = g.add(v, u);\n        if (id + 1 == (n - 1)*2) build();\n    }\n    void input(int\
    \ indexed = 1){\n        for (int i = 0; i < n - 1; i++){\n            int u,\
    \ v; cin >> u >> v;\n            u -= indexed, v -= indexed;\n            add_edge(u,\
...
辺の情報を標準入力から受け取って、辺を追加する。頂点番号は `indexed, indexed+1, ..., indexed+n-1` とする。
